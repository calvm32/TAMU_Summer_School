...
def solve_nonlinear(c, u_bcs, v_bcs, u_0, v_0, f, xs, ys, ts, stabilization=0, mu=0.5, lambd=0, bc_type="do_nothing"):
    ...
    U[1], V[1] = linear_forward_diff_step(c, U, V, 0, f, u_bcs, v_bcs, xs, ys, ts, stabilization, mu, lambd, bc_type)
    ...
    for n in range(1, total_times):
        ...
        U[n+1], V[n+1] = nonlinear_forward_diff_step(c, U, V, n, f, u_bcs, v_bcs, xs, ys, ts, stabilization, mu, lambd, bc_type)
...
def solve_linear(c, u_bcs, v_bcs, u_0, v_0, f, xs, ys, ts, stabilization=0, mu=0.5, lambd=0, bc_type="do_nothing"):
    ...
    for n in range(1, total_times):
        if n % 100 == 0:
            print(f"done w/ {n}/{total_times}")
        U[n+1], V[n+1] = linear_forward_diff_step(c, U, V, n, f, u_bcs, v_bcs, xs, ys, ts, stabilization, mu, lambd, bc_type)
...
if __name__ == "__main__":
    ...
    hx = 2**(-4) #(Omega[0][1] - Omega[0][0])/total_xpoints
    hy = 2**(-4) #(Omega[1][1] - Omega[1][0])/total_ypoints
    total_xpoints = int((Omega[0][1] - Omega[0][0])/hx)
    total_ypoints = int((Omega[1][1] - Omega[1][0])/hx)
    ...
    f = lambda t,x,y: np.array([[y*(1-y), 0], [0, 0]])

    # -------------------
    # boundary conditions
    # -------------------

    # available: dirichlet, do_nothing, reflecting, neumann, normal
    u_bc_type = {"dirichlet": ["left"]}
    v_bc_type = {"normal": ["top", "bottom", "right"]}
    ...
    U_nl, V_nl = solve_nonlinear(c, u_bcs, v_bcs, u_0, v_0, f, xs, ys, ts, stabilization, mu, lambd, bc_type)
    U_l, V_l = solve_linear(c, u_bcs, v_bcs, u_0, v_0, f, xs, ys, ts, stabilization, mu, lambd, bc_type)
    ...
    if plot:
        ...
        for ax in [ax1, ax2]:
            ax.set_xlim(Omega[0][0] - 0.1, Omega[0][1] + 0.5)
            ax.set_ylim(Omega[1][0] - 0.1, Omega[1][1] + 0.1)
            ax.set_aspect('equal')
