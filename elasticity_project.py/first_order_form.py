...
def step(U, V, n, xs, ts):
    ...
    for i in range(total_points+1):
        if i == 0:
            ...
            U_next[i] = U[n-1,i] - (tau/h)*(V[n,i+1] - V[n,i-1])
            V_next[i] = V[n-1,i] - (c**2)*(tau/h)*(U[n,i+1] - U[n,i-1])

    np.savetxt('output.txt', U, fmt='%.2f', delimiter=',')

    return U_next, V_next

def solve(c, alpha, beta, u_0, v_0, xs, ts):
...
if __name__ == "__main__":
    ...
    U, V = solve(c, alpha, beta, u_0, v_0, xs, ts)
